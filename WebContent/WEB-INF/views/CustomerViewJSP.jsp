...
<script type="text/javascript">
	function sendToPage() {
		...
		window.location.href = "CustomerFind?Name=".concat(input);
	}
...
<body>
	...
	<div class="container mt-5 mb-5">
		...
		<div class="d-flex justify-content-between mb-3">
			<span style="font-size: 24px">Khách hàng</span>
			...
			<button class="btn btn-success add"
				onclick="window.location.href='CustomerInsert?Size=<%=customers.size()%>'">Thêm
				khách hàng</button>
		</div>
		<div class="row g-2" style="margin-top: 30px">
			...
			<div class="col-md-3">
				<div class="card p-2 py-3 text-center">
					<div class="img mb-2">
						<img src="assets/img/user.png" width="70" class="rounded-circle">
					</div>
					<h5 class="mb-0">
						<%=customer.getNameCustomer()%>
					</h5>
					<small>SĐT: <%=String.valueOf(customer.getPhone())%>
					</small> <small>Địa chỉ: <%=String.valueOf(customer.getAddress())%>
					</small>

					<div class="mt-4 apointment">
						<button class="btn btn-success text-uppercase"
							style="background-color: #cc9966"
							onclick="window.location.href='CustomerDelete?Id=<%=customer.getIdCustomer()%>'">Xóa</button>
						<button class="btn btn-success text-uppercase"
							onclick="window.location.href='CustomerEdit?Id=<%=customer.getIdCustomer()%>&Name=<%=customer.getNameCustomer()%>&Address=<%=customer.getAddress()%>&Phone=<%=customer.getPhone()%>'">Edit</button>
					</div>
				</div>
			</div>
			<%
